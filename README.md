...
1. 录入六位起号、方向、每号有效印次（1~3）、计划纸张数（1~200）、
   严格升序且不重复的计划废张序号；
2. 需要抽查一段时，填写**核对起始/结束纸序**（留空默认覆盖计划全程），
   实测区随即只列出该区间内的纸张；
3. 在逐张实测区录入区间内每张的六位号码或 `SPOIL`（计划废张行会自动预填 `SPOIL`，可改）；
4. 提交后按纸序展示：纸序、类型、当前号码、有效印次进度、期望、实测、判定原因；
   横幅直接给出首个错号纸序与全部差异纸序（均为计划中的**绝对纸序**），差异行红色标出。
   区间从废张、同号中段或换号点开始时，首行期望与印次进度都承接前序纸张，而非重新起算。
...
`check_from` / `check_to` 为可选的核对区间（1-based 闭区间，计划中的绝对纸序）；
缺省的一端取计划边界，均不传即覆盖全程（旧请求行为不变）。后端始终先按完整计划
推进号码与有效印次，再截取该闭区间与实测逐张比对，因此 `actuals` 的数量必须等于
区间长度，返回的 `rows`、`mismatch_sheets`、`first_mismatch_sheet` 均使用绝对纸序，
并回显 `check_from` / `check_to`。

成功返回 `all_match`、`mismatch_sheets`、`first_mismatch_sheet` 以及逐张 `rows`。
任何参数/越界/区间/实测错误均返回 `400`：
...
`spoil_sheets` / `check_from` / `check_to` / `actuals`，前端据此在对应表单位置显示错误。
起止纸序超出计划、前后倒置或实测数量不等于区间长度时同样整单拒绝，不返回局部结果。
